...
void DrawLineCore(char* character, int pointAX, int pointAY, int pointBX, int pointBY, Color* fg, Color* bg, int thickness, bool isPanel, Panel panel) {
	if(thickness < 1) {
		return;
	}
	...
	while((termPos.x - pointBX) * aDir.x + (termPos.y - pointBY) * aDir.y <= 0)  {
		bool inScreen = termPos.x >= minBoundX && termPos.x <= maxBoundX && termPos.y >= minBoundY && termPos.y <= maxBoundY;
		...
		if(inScreen) {
			...
			for(int i = 1; i < thickness; i++) {
				...
				if(x1 >= minBoundX && x1 <= maxBoundX && y1 >= minBoundY && y1 <= maxBoundY) {
					DrawCharEx(character, x1, y1, fg, bg);
				}
				if(thickness > 1 && x2 >= minBoundX && x2 <= maxBoundX && y2 >= minBoundY && y2 <= maxBoundY) {
					DrawCharEx(character, x2, y2, fg, bg);
				}
			}
		}
...
void DrawRectangleCore(char* character, int posX, int posY, int width, int height, int originX, int originY, Color* fg, Color* bg, double rotation, float roundness, bool lines, int thicknessLines, bool aspectRatiofied, bool isPanel, Panel panel) {
...
void DrawCircleCore(char* character, int centerX, int centerY, int radius, double startAngle, double endAngle, Color* fg, Color* bg, bool lines, int thicknessLines, bool aspectRatiofied, bool isPanel, Panel panel) {
	double aspectRatio = aspectRatiofied ? ((GetCellProportions().x > 0 && GetCellProportions().y > 0) ? (double)GetCellProportions().x / (double)GetCellProportions().y : 0.5) : 1.0;
	...
	for(int y = startY; y <= endY; y++) {
		for(int x = startX; x <= endX; x++) {
			...
			if(!lines) {
				...
				if(fabs(distance - radius * radius) > thicknessLines * (radius * aspectRatio + (vWidth))) {
					continue;
				}
			}
			...
			bool inArc = (startAngle < endAngle) ? (angle >= startAngle && angle <= endAngle) : (angle >= startAngle || angle <= endAngle);
			if(!inArc) {
				continue;
			}
...
void DrawTriangleCore(char* character, int Ax, int Ay, int Bx, int By, int Cx, int Cy, int originX, int originY, Color* fg, Color* bg, double rotation, bool lines, int thicknessLines, bool aspectRatiofied, bool isPanel, Panel panel) {
	double aspectRatio = aspectRatiofied ? ((GetCellProportions().x > 0 && GetCellProportions().y > 0) ? (double)GetCellProportions().x / (double)GetCellProportions().y : 0.5) : 1.0;
	...
	int drawMinX = (minX > minBoundX) ? minX : minBoundX;
	int drawMaxX = (maxX < maxBoundX) ? maxX : maxBoundX;
	int drawMinY = (minY > minBoundY) ? minY : minBoundY;
	int drawMaxY = (maxY < maxBoundY) ? maxY : maxBoundY;
...
void DrawLinePro(char* character, int pointAX, int pointAY, int pointBX, int pointBY, Color* fg, Color* bg, int thickness) {
	DrawLineCore(character, pointAX, pointAY, pointBX, pointBY, fg, bg, thickness, false, (Panel){0});
}
...
void DrawRectangleEx(char* character, int x, int y, int width, int height, Color* fg, Color* bg, bool lines, int thicknessLines) {
	DrawRectanglePro(character, x, y, width, height, 0, 0, fg, bg, 0, 0.0f, lines, thicknessLines, true);
}
...
void DrawRectanglePro(char* character, int posX, int posY, int width, int height, int originX, int originY, Color* fg, Color* bg, double rotation, float roundness, bool lines, int thicknessLines, bool aspectRatiofied) {
	DrawRectangleCore(character, posX, posY, width, height, originX, originY, fg, bg, rotation, roundness, lines, thicknessLines, aspectRatiofied, false, (Panel){0});
}
...
void DrawCirclePro(char* character, int centerX, int centerY, int radius, double startAngle, double endAngle, Color* fg, Color* bg, bool lines, int thicknessLines, bool aspectRatiofied) {
	DrawCircleCore(character, centerX, centerY, radius, startAngle, endAngle, fg, bg, lines, thicknessLines, aspectRatiofied, false, (Panel){0});
}
...
void DrawTrianglePro(char* character, int Ax, int Ay, int Bx, int By, int Cx, int Cy, int originX, int originY, Color* fg, Color* bg, double rotation, bool lines, int thicknessLines, bool aspectRatiofied) {
	DrawTriangleCore(character, Ax, Ay, Bx, By, Cx, Cy, originX, originY, fg, bg, rotation, lines, thicknessLines, aspectRatiofied, false, (Panel){0});
}
